...
\begin{frame}{What is pLDDT and why does its \emph{structure} matter?}
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{tikzpicture}[x=0.10cm, y=0.037cm]
        % Light grid
  ...
  \begin{block}{Project goal}
    Reproduce the full Q4 fragmentation pipeline on the complete
    \textit{H.~sapiens} proteome (23\,391 AlphaFold-DB v4 fragments) and
    extend it with a novel 3D arity cross-correlation.
  \end{block}
\end{frame}
...
\begin{frame}{Five topological statistics}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}[x=0.052cm, y=0.70cm]
        % Light grid
...
\begin{frame}{Validation: three prototype proteins}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.47\textwidth}
      \textbf{Why are our $H_p$ values higher?}
      \vspace{4pt}
...
\begin{frame}{Proteome-scale Pearson correlation (Figure 7)}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \includegraphics[width=\textwidth]{../data/results/figure7_full.png}
    \end{column}
    \begin{column}{0.42\textwidth}
      \vspace{10pt}
...
\begin{frame}{The float-precision gap — unified explanation}
  ...
  \begin{tikzpicture}[
    ...
    \node[effect, below left=0.8cm and 0.5cm of root]  (e1) {More distinct\\persistence values\\in $\mathcal{P}_D$};
    \node[effect, below right=0.8cm and 0.5cm of root] (e2) {Finer $N_{cc}$ curve\\more local\\oscillations};
    \node[cause, below=0.9cm of e1]  (c1) {$H_p$ inflated\\(+0.14 to +0.33)};
    \node[cause, below=0.9cm of e2]  (c2) {More PLM peaks\\survive $t_\nu$};
    \node[effect, below=0.7cm of c1] (r1) {Pearson $r$:\\0.864 vs 0.97};
    \node[effect, below=0.7cm of c2] (r2) {Candidates:\\183 vs 86};
